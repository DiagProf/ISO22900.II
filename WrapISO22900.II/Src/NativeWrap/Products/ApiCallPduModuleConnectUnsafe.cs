...
namespace ISO22900.II
{
    ...
    internal class ApiCallPduModuleConnectUnsafe : ApiCallPduModuleConnect
    {
        internal override unsafe void PduModuleConnect(uint moduleHandle)
        {
            CheckResultThrowException(PDUModuleConnect(moduleHandle));
        }

        internal ApiCallPduModuleConnectUnsafe(IntPtr handleToLoadedNativeLibrary) : base(handleToLoadedNativeLibrary)
        {
            
        }

        //should look like the C function as much as possible.
        private unsafe PduError PDUModuleConnect(UNUM32 hMod) => ((delegate* unmanaged[Stdcall]<UNUM32, PduError>)AddressOfNativeMethod)(hMod);
