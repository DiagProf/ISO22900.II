...
namespace ISO22900.II
{
    ...
    internal class ApiCallPduModuleDisconnectUnsafe : ApiCallPduModuleDisconnect
    {
        internal override void PduModuleDisconnect(uint moduleHandle)
        {
            CheckResultThrowException(PDUModuleDisconnect(moduleHandle));
        }

        internal ApiCallPduModuleDisconnectUnsafe(IntPtr handleToLoadedNativeLibrary) : base(handleToLoadedNativeLibrary)
        {
        }

        //should look like the C function as much as possible.
        private unsafe PduError PDUModuleDisconnect(UNUM32 hMod) => ((delegate* unmanaged[Stdcall]<UNUM32, PduError>)AddressOfNativeMethod)(hMod);
    }
}
